...
class BlurEditorState extends State<BlurEditor>
  ...
  // The adjusted image variable
  late EditorImage adjustedImage;

  // Stream for UI rebuild when blur factor changes
  late final StreamController<void> _uiBlurStream;

  // The selected blur state
  late double blurFactor;

  @override
  void initState() {
    adjustedImage = widget.editorImage; // Initially, use the editorImage
  ...
  /// Function to apply image adjustments (e.g., brightness, contrast)
  void applyImageAdjustments() {
    // You should implement logic to apply filters or changes to the image
    // Example logic: adjustedImage = applyFilters(widget.editorImage, filterMatrix);
    adjustedImage = widget.editorImage; // Replace with actual adjustment logic
  }

  /// Function for handling changes in the blur factor
  ...
  /// Handle changes end for blur factor
